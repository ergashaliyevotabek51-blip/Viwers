...
# ADMIN ID lar ro'yxati - o'zingizning ID ni yozing
ADMIN_IDS = [
    "774440841, 7818576058",  # O'ZINGIZNING TELEGRAM ID
]
...
async def send_movie_by_query(query, context, movie_code: str, user_id: str):
    ...
    try:
        ...
        logger.error(f"Send movie by query error: {e}")
    ...
    try:
        ...
        if update.message.forward_from_chat:
            ...
            await update.message.reply_text(
                ...
                f"🎭 Janr: {genre_display}",
                reply_markup=get_admin_keyboard(str(update.effective_user.id))
            )
    except Exception as e:
        logger.error(f"Process add movie error: {e}")
        await update.message.reply_text("❌ Xatolik yuz berdi!")
...
async def process_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("❌ Broadcast xatolik!")
...
async def process_add_limit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.error(f"Add limit error: {e}")
...
async def create_backup(query):
    ...
    try:
        ...
        await query.answer("❌ Backup xatolik!", show_alert=True)
...
def main():
    ...
    if BOT_TOKEN == "YOUR_BOT_TOKEN_HERE" or len(BOT_TOKEN) < 20:
        print("❌ ERROR: Haqiqiy BOT_TOKEN kiriting!")
        return
    
    if not ADMIN_IDS or ADMIN_IDS[0] == "123456789":
        print("⚠️ WARNING: Haqiqiy ADMIN_ID kiriting!")
        print("O'zingizning ID ni bilish uchun @userinfobot ga /start yozing")
